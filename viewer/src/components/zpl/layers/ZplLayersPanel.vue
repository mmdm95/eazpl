...
<template>
  <section class="flex h-64 min-h-0 shrink-0 flex-col gap-2 border-t border-border pt-3">
    ...
    <ul v-else class="flex-1 space-y-1 overflow-y-auto pr-1">
      ...
      >
        <button type="button" class="min-w-0 flex-1 text-start" @click="emit('select', component.id)">
        ...
        <div class="flex items-center gap-0.5">
          <button :class="actionClass" type="button" :aria-label="`${component.visible === false ? 'Show' : 'Hide'} ${component.type}`" @click.stop="emit('toggle-visibility', component.id)">
            <component :is="component.visible === false ? EyeOff : Eye" class="h-3.5 w-3.5" />
          </button>
          <button :class="actionClass" type="button" :aria-label="`${component.locked ? 'Unlock' : 'Lock'} ${component.type}`" @click.stop="emit('toggle-lock', component.id)">
            <component :is="component.locked ? Lock : LockOpen" class="h-3.5 w-3.5" />
          </button>
          <button :class="actionClass" type="button" :aria-label="`Move ${component.type} up`" @click.stop="emit('move', component.id, 1)">
            <ArrowUp class="h-3.5 w-3.5" />
          </button>
          <button :class="actionClass" type="button" :aria-label="`Move ${component.type} down`" @click.stop="emit('move', component.id, -1)">
            <ArrowDown class="h-3.5 w-3.5" />
          </button>
          <button :class="actionClass" type="button" :aria-label="`Duplicate ${component.type}`" @click.stop="emit('duplicate', component.id)">
            <Copy class="h-3.5 w-3.5" />
          </button>
          <button :class="actionClass" type="button" :aria-label="`Delete ${component.type}`" @click.stop="emit('remove', component.id)">
