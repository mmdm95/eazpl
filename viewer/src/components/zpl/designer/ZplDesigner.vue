...
const selectedDefinition = computed(
  () =>
    (designer.selectedComponent.value && definitionMap.value.get(designer.selectedComponent.value.type)) ?? null,
)
...
async function generate(): Promise<void> {
  ...
  try {
    generatedZpl.value = await generateZpl(designer.state.value)
...
<template>
  <main class="flex h-screen w-full flex-col overflow-hidden bg-surface-muted text-content">
    ...
    <div class="grid min-h-0 flex-1 grid-cols-[280px_minmax(0,1fr)_340px]">
      ...
      <aside class="flex min-h-0 flex-col gap-4 overflow-y-auto border-s border-border bg-surface p-4">
        <ZplPropertiesPanel
        ...
        <section class="flex min-h-64 flex-1 flex-col">
          <h2 class="mb-3 text-sm font-semibold uppercase tracking-wide text-content-muted">ZPL preview</h2>
          <pre
